            ...
            <div class="card-body p-3 d-flex flex-column align-items-center justify-content-center">
                ...
                @else
                    ...
                    @if ( Auth::user()->type ==1)
                        <div class="d-flex flex-wrap justify-content-center col-12">
                            @if ( $yacomprada > 0)
                                <button type="button" onClick="deleteDataPurchase({{ $yacomprada }})" class="btn btn-primary  btn-sm m-1 col-lg-8 col-md-12">
                                    Anular compra
                                </button>
                            @else
                                <button type="button" onClick="postDataPurchase({{ $app->id}})" class="btn btn-primary  btn-sm m-1 col-lg-8 col-md-12">
                                    Comprar!
                                </button>
                                @if ( $yadeseada > 0)
                                    <button type="button" onClick="deleteDataWish({{ $yadeseada}})" class="btn btn-primary  btn-sm m-1 col-lg-8 col-md-12">
                                        Quitar de la lista de deseos
                                    </button>
                                @else
                                    <button type="button" onClick="postDataWish({{ $app->id}})" class="btn btn-primary  btn-sm m-1 col-lg-8 col-md-12">
                                        Agregar a la lista de deseos
                                    </button>
                                @endif
                            @endif
